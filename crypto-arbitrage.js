  ...
  for (const exchange of Object.values(EXCHANGES)) {
    if ('apiSymbols' in exchange) {
      ...
      for (const currency of exchange.apiSymbols.currencies) {
        listTriangularArbitrage(exchange, false, currency, symbols);
      }
      for (const intermediateCoin of exchange.apiSymbols.intermediateCoins) {
        listTriangularArbitrage(exchange, true, intermediateCoin, symbols);
      }
    }
  }
}
...
async function listTriangularArbitrage (exchange, isIntermediateCoin, intm, symbols) {
  const baseIntmBidAndAsk = !isIntermediateCoin ? await getBidAndAskFromExchange(exchange.getApiOrderBookSymbol(`${base}-${intm}`), exchange) : [];
  const intmBaseBidAndAsk = isIntermediateCoin ? await getBidAndAskFromExchange(exchange.getApiOrderBookSymbol(`${intm}-${base}`), exchange) : [];
  for (const symbol of symbols) {
    if (symbol[exchange.apiSymbols.baseAsset] !== base && symbol[exchange.apiSymbols.quoteAsset] === intm) {
      ...
      const trgtBaseBidAndAsk = await getBidAndAskFromExchange(exchange.getApiOrderBookSymbol(`${trgt}-${base}`), exchange);
      const trgtIntmBidAndAsk = await getBidAndAskFromExchange(exchange.getApiOrderBookSymbol(`${trgt}-${intm}`), exchange);
