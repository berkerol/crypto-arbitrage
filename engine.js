...
function calculateProfit (exchangeDetails, type, intermediate, coin, method, final, minSize) {
  if (isFinite(final)) {
  ...
  for (const symbol of symbols) {
    if (symbol[exchange.apiSymbols.baseAsset] !== base && symbol[exchange.apiSymbols.quoteAsset] === intm) {
      ...
      if (trgtBaseBidAndAsk && trgtIntmBidAndAsk) {
        ...
        calculateProfit(exchange, !isIntermediateCoin ? 'currency' : 'intermediate coin', intm, trgt, 'method1', final1, minSize1);
        ...
        calculateProfit(exchange, !isIntermediateCoin ? 'currency' : 'intermediate coin', intm, trgt, 'method2', final2, minSize2);
      }
    }
  }
}
